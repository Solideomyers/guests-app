...
const StatsCard: React.FC<StatsCardProps> = ({ title, value, colorClass }) => {
  ...
  return (
    <div className='bg-card text-card-foreground p-4 sm:p-5 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 flex items-center border border-border'>
      <div
        className={`rounded-full p-2 sm:p-3 mr-3 sm:mr-4 ${colorClass} transition-all`}
      >
        {icon()}
      </div>
      <div>
