...
const App: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const confirmed = guests.filter(
      (g) => g.status === AttendanceStatus.CONFIRMED
    ).length;
    const pending = guests.filter(
      (g) => g.status === AttendanceStatus.PENDING
    ).length;
    const declined = guests.filter(
      (g) => g.status === AttendanceStatus.DECLINED
    ).length;
  ...
  const handleUpdateGuestStatus = useCallback(
  ...
  const handleTogglePastorStatus = useCallback(
    (id: number, isPastor: boolean) => {
      setGuests((prevGuests) =>
        prevGuests.map((guest) =>
          guest.id === id ? { ...guest, isPastor } : guest
        )
      );
    },
    []
  ...
  const handleSaveGuest = (
    guestData: Omit<Guest, 'registrationDate' | 'status'>
  ) => {
    ...
    if ('id' in guestData && guestData.id) {
      setGuests(
        guests.map((g) => (g.id === guestData.id ? { ...g, ...guestData } : g))
      );
    } else {
  ...
  const handleDeleteGuest = useCallback(
    (id: number) => {
      if (
        window.confirm('¿Estás seguro de que quieres eliminar a este invitado?')
      ) {
  ...
  const handleBulkDelete = () => {
    if (
      window.confirm(
        `¿Estás seguro de que quieres eliminar a ${selectedGuests.size} invitados?`
      )
    ) {
  ...
  const handleBulkStatusChange = (status: AttendanceStatus) => {
    setGuests(
      guests.map((g) => (selectedGuests.has(g.id) ? { ...g, status } : g))
    );
    setSelectedGuests(new Set());
  };
  ...
  const handleBulkSetPastorStatus = (isPastor: boolean) => {
    setGuests(
      guests.map((g) => (selectedGuests.has(g.id) ? { ...g, isPastor } : g))
    );
    setSelectedGuests(new Set());
  };
  ...
  const handleExportPDF = () => {
    ...
    const currentStats = {
      ...
      confirmed: sortedGuests.filter(
        (g) => g.status === AttendanceStatus.CONFIRMED
      ).length,
      pending: sortedGuests.filter((g) => g.status === AttendanceStatus.PENDING)
        .length,
      declined: sortedGuests.filter(
        (g) => g.status === AttendanceStatus.DECLINED
      ).length,
    };
